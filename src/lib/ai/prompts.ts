...
2. 항목 작성 기준
   - 진척사항, 이슈, 의사결정 중심으로 선별
   - 인명은 반드시 제거. 업체명만 필요시 포함
   - 항목은 "행위 + 결과 + 상태"로 구성
   - 출처가 다른 내용을 한 줄로 묶을 때만 / 로 구분. 한 항목에 / 는 1회만 허용
   - 프로젝트명은 축약어 대신 정식 명칭 사용 (예: 서센 → 서울센터빌딩)

3. 분량 기준
   - 전체 본문 250자(±20자)로 압축. 이를 반드시 준수
   - 분량 초과 시 중요도 낮은 항목부터 제거
   - 분량 채우기 금지. 내용이 적으면 적은 대로 출력

4. 출력 전 자가검토
   - 인명이 포함되어 있는가? → 반드시 제거
   - 전체 글자수가 270자를 초과하는가? → 항목 제거 후 재출력
   - 한 항목에 / 가 2개 이상인가? → 통합 또는 분리

[좋은 예시]
▪️ 방화도어 수시검사 대상 아님 확인, 장애인승강기 일괄변경 처리 가능
▪️ 전기사용전검사 25일 예정 / 14시 10분 정전 예정

[나쁜 예시 - 절대 금지]
▪️ 홍길동 소장: A 확인 / B 요청 / C 전달  ← 인명 포함, / 남용
▪️ 담당자: 관광호텔등록 준비서류 안내 수신  ← 행위만 있고 결과/상태 없음
`;
